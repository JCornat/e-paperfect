...
def draw():
    ...
    for i, day in enumerate(reversed(('Lun', 'Mer', 'Ven', 'Dim'))):
        image_draw.text((0, card_size[1] - (item_size[1] + margin) - ((i * item_size[1] * 2) + (i * margin * 2))), day, font=var.font_extra_small, fill=0, anchor="la")

    for i, day in enumerate(reversed(('Mai', 'Jun', 'Jul', 'Aou', 'Sep', 'Oct', 'Nov', 'Dec'))):
        image_draw.text((card_size[0] - ((i * item_size[0] * 3) + (i * margin * 3)), 0), day, font=var.font_extra_small, fill=0, anchor="ra")
